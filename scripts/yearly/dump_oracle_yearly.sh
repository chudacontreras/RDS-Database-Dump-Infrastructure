...
BEGIN
  ...
  v_hdnl := DBMS_DATAPUMP.OPEN(
    ...
    job_name  => 'YEARLY_EXPORT_${TIMESTAMP}'
  );
  ...
  DBMS_DATAPUMP.ADD_FILE(
    handle    => v_hdnl,
    filename  => '${DB_IDENTIFIER}_yearly_${TIMESTAMP}.log',
    directory => 'DATA_PUMP_DIR',
...
BEGIN
  rdsadmin.rdsadmin_util.upload_to_s3(
    ...
    p_s3_prefix      => 'oracle/${DB_IDENTIFIER}/yearly/',
...
if [[ ${SQLPLUS_RC} -ne 0 ]]; then
  log "WARN: rdsadmin.upload_to_s3 no disponible. Intentando con rdsadmin_s3_tasks..."
...
BEGIN
  v_task_id := rdsadmin.rdsadmin_s3_tasks.upload_to_s3(
    p_bucket_name    => '${S3_BUCKET}',
    p_prefix         => 'oracle/${DB_IDENTIFIER}/yearly/',
    p_s3_prefix      => 'oracle/${DB_IDENTIFIER}/yearly/',
    p_directory_name => 'DATA_PUMP_DIR'
  );
...
BEGIN
  ...
  UTL_FILE.FREMOVE('DATA_PUMP_DIR', '${DB_IDENTIFIER}_yearly_${TIMESTAMP}.log');
END;
/
EXIT;
EOF
...
S3_KEY="oracle/${DB_IDENTIFIER}/yearly/${DUMP_RDS_FILE}"
